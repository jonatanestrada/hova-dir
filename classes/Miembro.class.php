...
public function editMiembro( $datos ){
	...
	$sql = "UPDATE miembros SET fecha_update = CURRENT_TIMESTAMP, user_update = '".$this->idUser."', sexo = '".$datos['sexo']."', preferencia_nombre_sec = '".$preferencia_nombre_sec."', preferencia_nombre = '".$preferencia_nombre."', fecha_ingreso = ".$fecha_ingreso.", fecha_nacimiento = ".$fecha_nacimiento.", nombre = '".html_entity_decode($datos['nombre'])."', nombre_sec = '".html_entity_decode($datos['nombre_sec'])."', apaterno = '".html_entity_decode($datos['apaterno'])."', amaterno = '".html_entity_decode($datos['amaterno'])."', email = '".$datos['email']."', 
